...
BEGIN
    -- testcomun@test.com (Password: comun)
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'testcomun@test.com') THEN
        INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at, raw_app_meta_data, raw_user_meta_data, created_at, updated_at)
        VALUES (uid1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'testcomun@test.com', crypt('comun', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now());
    ...
    -- testoro@test.com (Password: oro)
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'testoro@test.com') THEN
        INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at, raw_app_meta_data, raw_user_meta_data, created_at, updated_at)
        VALUES (uid2, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'testoro@test.com', crypt('oro', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now());
    ...
    -- testplatino@test.com (Password: platino)
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'testplatino@test.com') THEN
        INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at, raw_app_meta_data, raw_user_meta_data, created_at, updated_at)
        VALUES (uid3, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'testplatino@test.com', crypt('platino', gen_salt('bf')), now(), '{"provider":"email","providers":["email"]}', '{}', now(), now());
